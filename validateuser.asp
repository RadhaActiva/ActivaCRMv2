...
If Not rs.EOF Then
  ...
  if rs("user_type") = "Riegen" then
	  ...
	  if rs("accesslevel") = "mis" then 
	     Response.Redirect("core/mis_home.asp")
		 	  
	  elseif rs("accesslevel") = "finance" then
	     Response.Redirect("core/rm_home.asp")	 
		 
	  elseif rs("accesslevel") = "cs" or rs("accesslevel") = "sc" then
	     Response.Redirect("core/rm_home.asp")	 	
		 
	  elseif rs("accesslevel") = "technician" then
	     Response.Redirect("core/rm_home.asp")
	  
	  elseif rs("accesslevel") = "technician2" then
	     Response.Redirect("core/rm_home.asp") 	 	
		 
	  elseif rs("accesslevel") = "sales" then
	     Response.Redirect("core/rm_home.asp")	 		 	 
   ...
   if Request.Cookies("GAPS")("logintry") = Request.Form("txtID") then 	          
	  ...
	  if cint(Request.Cookies("GAPS")("logincount")) >= 3 then
			 'sql = "Update users set user_active = 'N' where user_name = '"& ChkString(Trim(Request("txtID"))) &"'"
			 'CUD(sql)
...
if request("type") = "forgetpassword" then         
   ...
   if spassword <> "" then   
    ...
    '  Response.Redirect "forgetpassword.asp?loginerr=Your Username and Password has been sent to " & email 
