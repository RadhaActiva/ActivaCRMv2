...
<table border="0" cellpadding="3" cellspacing="0" bordercolor="#CCCCCC">
  ...
  <tr> 
    <td class="style21"><form name="form1" method="post" action="rm_job_new_schedule.asp?searchitem=<%=searchitem%>&<%=searchvalue%>&<%=searchvalue%>&md_type=<%=md_type%>&formname=<%=formname%>&fieldname=<%=fieldname%>">
...
%>
        <tr valign="top" bgcolor="<%=nbgcolor%>"> 
          <td nowrap><%=i%>.</td>
          <td><a href="javascript:parent.opener.document.forms['formorder'].job_tech_code.value='<%=rs("tech_code")%>';parent.opener.document.forms['formorder'].job_appointment_date.value='<%=job_date%>';parent.window.close();"><%=rs("tech_code")%></a></td>
          <td><%=rs("tech_name")%></td>
