...
<table border="0" cellpadding="3" cellspacing="0" bordercolor="#CCCCCC">
  ...
  <tr> 
    <td class="style21"><form name="form1" method="post" action="rm_do_new_model.asp">
...
<select name="md_type" id="md_type">
  ...
  <tr> 
	...
	For j = 1 To pgCount
				...
				Else
					Response.Write " <a href='rm_do_new_model.asp?num=" & (j-1) * row & link & "'>"& j &"</a>"
				End If
	...
	If Remain > row Then
	  Response.Write "<a href='rm_do_new_model.asp?num=" & Showed+row & link & "'> Next >></a>"
	End If
...
%>
        <tr valign="top" bgcolor="<%=nbgcolor%>"> 
          <td nowrap><%=count%>.</td>
          <td><a href="javascript:parent.opener.document.forms['<%=formname%>'].<%=fieldname%>.value='<%=rs("md_code")%>';parent.opener.document.forms['<%=formname%>'].<%=fieldname1%>.value='<%=ChkString(rs("md_desc"))%>';parent.window.close();"><%=rs("md_code")%></a></td>
          <td><%=rs("md_desc")%></td>
