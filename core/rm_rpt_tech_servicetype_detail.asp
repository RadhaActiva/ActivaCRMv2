...
job_reportedby =request("job_reportedby")
...
sql2 = "SELECT tbljob.job_id, tbljob.job_code, tbljob.job_count, tbljob.job_date, tbljob.job_cust_code, tbljob.job_cust_name, tbljob.job_cust_address, " & _
		...
		"tbljob.job_submitteddate, tbljob.job_donedate, tbljob.job_posteddate, tbljob.job_cancelleddate " & _
...
if jobtype="status" and job_status<>"all" then 
   sql2 = sql2 & " and tbljob.job_status in ('" & job_status & "') "

elseif jobtype="job_reportedby" and job_reportedby<>"all" then 
    sql2 = sql2 & " and tbljob.job_reportedby in ('" & job_reportedby & "') "
...
count = count + Showed
link = "&job_date_from=" & job_date_from & "&job_date_to=" & job_date_to & "&job_status=" & job_status & "&jobtype=" & jobtype & "&job_reportedby=" & job_reportedby & "&job_tech_code=" & job_tech_code   
%>
<table border="0" cellpadding="3" cellspacing="0" bordercolor="#CCCCCC">
  <tr> 
    ...
    <td align="right" class="style21"><span class="titlegrey1"><a href="rm_rpt_tech_servicetype_detail_excel.asp" target="_blank"><img src="images/excel.jpg" width="57" height="21" border="0" /></a></span></td>
  </tr>
  <tr> 
	...
	For j = 1 To pgCount
				...
				Else
					Response.Write " <a href='rm_rpt_tech_servicetype_detail.asp?num=" & (j-1) * row & link & "'>"& j &"</a>"
				End If
	...
	If Remain > row Then
	  Response.Write "<a href='rm_rpt_tech_servicetype_detail.asp?num=" & Showed+row & link & "'> Next >></a>"
	End If
