...
'----------------------------------------------------------------------------------------------------    
  Case "rpt_farmonth_year_reset"
	...
	response.write "tblrpr_farmonth reset done"
	
	url = "action_report_farmonthyear_return.asp?type=rpt_farmonth_year&orderby=" & orderby & "&ordertype=" & ordertype & "&jobmonth=" & jobmonth & "&jobyear=" & jobyear & "&TotalSales=" & request("TotalSales") & "&job_tech_model=" & job_tech_model & "&job_tech_type=" & job_tech_type & "&stepno=1"	
...
'----------------------------------------------------------------------------------------------------    
  Case "rpt_farmonth_year"
	...
	sql1 = "SELECT top 1 id, faulth_code, faulth_desc, fa_month1_over, fa_month1_under, fa_month2_over, fa_month2_under, fa_month3_over, fa_month3_under, " & _
		   ...
		   "FROM tblrpr_farmonth where fa_done='N' "
	...
	sql2 = "SELECT count(tbljob.job_id) as totaljob FROM tbljob left join tbltechnician on tbljob.job_tech_code = tbltechnician.tech_code " & _
		  ...
		  "and tbljob.job_tech_faulty_code='" & rs1("faulth_code") & "' and job_actual_wrty_status='Over' and tbljob.job_status='Posted' and tbljob.job_hq_category_code <> 'IT' "
    ...
    sql2 = "SELECT count(tbljob.job_id) as totaljob FROM tbljob left join tbltechnician on tbljob.job_tech_code = tbltechnician.tech_code " & _
	      "where tbljob.job_id is not null " & _
		  "and  month(tbljob.job_posteddate) = '" & jobmonth1 & "' and year(tbljob.job_posteddate) = '" & jobyear1 & "' " & _
		  "and tbljob.job_tech_faulty_code='" & rs1("faulth_code") & "' and job_actual_wrty_status='Under' and tbljob.job_status='Posted' and tbljob.job_hq_category_code <> 'IT' "
	if job_tech_type <> "" then 
	   sql2 = sql2 & " and job_tech_type = '" & job_tech_type & "' "
	end if
	if job_tech_model <> "" then 
	...
	for i = 1 to 12 
		sql2 = "SELECT count(tbljob.job_id) as totaljob FROM tbljob left join tbltechnician on tbljob.job_tech_code = tbltechnician.tech_code " & _
			  ...
			  "and tbljob.job_tech_faulty_code='" & rs1("faulth_code") & "' and job_hq_category_code='MD' and job_actual_wrty_status='Over' and tbljob.job_status='Posted' "
		if job_tech_type <> "" then 
		   sql2 = sql2 & " and job_tech_type = '" & job_tech_type & "' "
		end if
		if job_tech_model <> "" then 
	...
	for i = 1 to 12 
		sql2 = "SELECT count(tbljob.job_id) as totaljob FROM tbljob left join tbltechnician on tbljob.job_tech_code = tbltechnician.tech_code " & _
			  "where tbljob.job_id is not null " & _
			  "and  month(tbljob.job_posteddate) = '" & jobmonth_list(i) & "' and year(tbljob.job_posteddate) = '" & jobyear_list(i) & "' " & _
			  "and tbljob.job_tech_faulty_code='" & rs1("faulth_code") & "' and job_hq_category_code='MD' and job_actual_wrty_status='Under' and tbljob.job_status='Posted'"
		if job_tech_type <> "" then 
		   sql2 = sql2 & " and job_tech_type = '" & job_tech_type & "' "
		end if
		if job_tech_model <> "" then 
	...
	for i = 1 to 12  
		sql2 = "SELECT count(tbljob.job_id) as totaljob FROM tbljob left join tbltechnician on tbljob.job_tech_code = tbltechnician.tech_code " & _
			  "where tbljob.job_id is not null " & _
			  "and  month(tbljob.job_posteddate) = '" & jobmonth_list(i) & "' and year(tbljob.job_posteddate) = '" & jobyear_list(i) & "' " & _
			  "and tbljob.job_tech_faulty_code='" & rs1("faulth_code") & "' and job_hq_category_code='DS' and job_actual_wrty_status='Over' and tbljob.job_status='Posted'"
		if job_tech_type <> "" then 
		   sql2 = sql2 & " and job_tech_type = '" & job_tech_type & "' "
		end if
		if job_tech_model <> "" then 
	...
	for i = 1 to 12  
		sql2 = "SELECT count(tbljob.job_id) as totaljob FROM tbljob left join tbltechnician on tbljob.job_tech_code = tbltechnician.tech_code " & _
			  "where tbljob.job_id is not null " & _
			  "and  month(tbljob.job_posteddate) = '" & jobmonth_list(i) & "' and year(tbljob.job_posteddate) = '" & jobyear_list(i) & "' " & _
			  "and tbljob.job_tech_faulty_code='" & rs1("faulth_code") & "' and job_hq_category_code='DS' and job_actual_wrty_status='Under' and tbljob.job_status='Posted'"
		if job_tech_type <> "" then 
		   sql2 = sql2 & " and job_tech_type = '" & job_tech_type & "' "
		end if
		if job_tech_model <> "" then 
	...
	for i = 1 to 12  
		sql2 = "SELECT count(tbljob.job_id) as totaljob FROM tbljob left join tbltechnician on tbljob.job_tech_code = tbltechnician.tech_code " & _
			  "where tbljob.job_id is not null " & _
			  "and  month(tbljob.job_posteddate) = '" & jobmonth_list(i) & "' and year(tbljob.job_posteddate) = '" & jobyear_list(i) & "' " & _
			  "and tbljob.job_tech_faulty_code='" & rs1("faulth_code") & "' and job_hq_category_code='WI' and job_actual_wrty_status='Over' and tbljob.job_status='Posted'"
		if job_tech_type <> "" then 
		   sql2 = sql2 & " and job_tech_type = '" & job_tech_type & "' "
		end if
		if job_tech_model <> "" then 
	...
	for i = 1 to 12  
		sql2 = "SELECT count(tbljob.job_id) as totaljob FROM tbljob left join tbltechnician on tbljob.job_tech_code = tbltechnician.tech_code " & _
			  "where tbljob.job_id is not null " & _
			  "and  month(tbljob.job_posteddate) = '" & jobmonth_list(i) & "' and year(tbljob.job_posteddate) = '" & jobyear_list(i) & "' " & _
			  "and tbljob.job_tech_faulty_code='" & rs1("faulth_code") & "' and job_hq_category_code='WI' and job_actual_wrty_status='Under' and tbljob.job_status='Posted'"
		if job_tech_type <> "" then 
		   sql2 = sql2 & " and job_tech_type = '" & job_tech_type & "' "
		end if
		if job_tech_model <> "" then 
	...
	for i = 1 to 12  
		sql2 = "SELECT count(tbljob.job_id) as totaljob FROM tbljob left join tbltechnician on tbljob.job_tech_code = tbltechnician.tech_code " & _
			   "where tbljob.job_id is not null " & _
			   "and  month(tbljob.job_posteddate) = '" & jobmonth_list(i) & "' and year(tbljob.job_posteddate) = '" & jobyear_list(i) & "' " & _
			   "and tbljob.job_tech_faulty_code='" & rs1("faulth_code") & "' and job_hq_category_code='CF' and job_actual_wrty_status='Over' and tbljob.job_status='Posted'"
		if job_tech_type <> "" then 
		   sql2 = sql2 & " and job_tech_type = '" & job_tech_type & "' "
		end if
		if job_tech_model <> "" then 
    ...
    sql2 = "SELECT count(tbljob.job_id) as totaljob FROM tbljob left join tbltechnician on tbljob.job_tech_code = tbltechnician.tech_code " & _
	      "where tbljob.job_id is not null " & _
		  "and  month(tbljob.job_posteddate) = '" & jobmonth_list(i) & "' and year(tbljob.job_posteddate) = '" & jobyear_list(i) & "' " & _
		  "and tbljob.job_tech_faulty_code='" & rs1("faulth_code") & "' and job_hq_category_code='CF' and job_actual_wrty_status='Under' and tbljob.job_status='Posted'"
	if job_tech_type <> "" then 
	   sql2 = sql2 & " and job_tech_type = '" & job_tech_type & "' "
	end if
	if job_tech_model <> "" then 
	...
	for i = 1 to 12  
		...
		'fa_IT_over = fa_IT_over + chknumber(fa_IT_over_month(i))
		'fa_IT_under = fa_IT_under + chknumber(fa_IT_under_month(i))
	next
	...
	if stillgotRecord = "No" then 
	   ...
	   response.write "Process Completed, <a href='rm_rpt_farmonth_year.asp?orderby=" & orderby & "&ordertype=" & ordertype & "&jobmonth=" & jobmonth & "&jobyear=" & jobyear & "&TotalSales=" & TotalSales & "&job_tech_model=" & job_tech_model & "&job_tech_type=" & job_tech_type & "&loginerr=Job has been updated.#articletitle'>View Result</a>"
	   response.end
	
	else
	
	   url = "action_report_farmonthyear_return.asp?type=rpt_farmonth_year&orderby=" & orderby & "&ordertype=" & ordertype & "&jobmonth=" & jobmonth & "&jobyear=" & jobyear & "&TotalSales=" & TotalSales & "&job_tech_model=" & job_tech_model & "&job_tech_type=" & job_tech_type & "&stepno=2"	
