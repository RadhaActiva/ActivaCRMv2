...
job_date = chkdate(job_date)
job_date = replace(job_date, "-","")
...
smscontent = "J:" & job_code & chr(13) & " " & job_date & chr(13) & job_cust_name & chr(13) & "T:" & job_cust_tel1 & chr(13) & "/" & job_cust_tel2 & chr(13) & _
             "M:" & job_model_desc & chr(13) & job_cust_address & " " & job_cust_city & " " & _
			  job_cust_postcode & " " & state_code

smsnumber = request("smsnumber")
smsnumber = replace(smsnumber, "-","")
smsnumber = replace(smsnumber, " ","")
smsnumber = "6" & smsnumber

response.write "SMS To: <b>" & smsnumber & "</b><br>"
response.write "SMS Content: <b>" & smscontent & "</b><br><br>"
response.write "SMS Sent succefully.<br>"
%>
    ...
    <body>
<iframe src="rm_jobsheet_sendsms_iframe.asp?job_code=<%=job_code%>&smsnumber=<%=smsnumber%>" width="100" height="60" frameborder="0" scrolling="no"></iframe>
    </body>
    </html>