...
<table border="0" cellpadding="3" cellspacing="0" bordercolor="#CCCCCC">
  <tr> 
    ...
    <td width="88" align="center" class="style21"><a href="rm_rpt_fararea_detail_excel.asp?job_date_from=<%=job_date_from%>&job_date_to=<%=job_date_to%>&faulth_code=<%=faulth_code%>&job_tech_type=<%=job_tech_type%>&job_tech_model=<%=job_tech_model%>&stype=<%=stype%>&state_id=<%=state_id%>" target="_blank"><img src="images/excel.jpg" width="57" height="21" border="0"></a></td>
  </tr>
  <tr> 
	...
	For j = 1 To pgCount
				...
				Else
					Response.Write " <a href='rm_rpt_fararea_detail.asp?num=" & (j-1) * row & link & "'>"& j &"</a>"
				End If
	...
	If Remain > row Then
	  Response.Write "<a href='rm_rpt_fararea_detail.asp?num=" & Showed+row & link & "'> Next >></a>"
	End If
