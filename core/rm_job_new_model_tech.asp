...
<%
i = 1
sql = "SELECT md_id, md_code, md_desc, md_category, md_type FROM tblmodel where md_id is not null and " & searchitem & " like '%" & searchvalue& "%' "
...
<table border="0" cellpadding="3" cellspacing="0" bordercolor="#CCCCCC">
  ...
  <tr> 
    <td class="style21"><form name="form1" method="post" action="rm_job_new_model_tech.asp">
...
<select name="md_type" id="md_type">
  ...
  <tr> 
	...
	For j = 1 To pgCount
				...
				Else
					Response.Write " <a href='rm_job_new_model_tech.asp?num=" & (j-1) * row & link & "'>"& j &"</a>"
				End If
	...
	If Remain > row Then
	  Response.Write "<a href='rm_job_new_model_tech.asp?num=" & Showed+row & link & "'> Next >></a>"
	End If
...
%>
        <tr valign="top" bgcolor="<%=nbgcolor%>"> 
          <td nowrap><%=count%>.</td>
          <td><a href="javascript:parent.opener.document.forms['formorder2'].job_tech_model.value='<%=rs("md_code")%>';parent.opener.document.forms['formorder2'].job_tech_model_desc.value='<%=chkstring(rs("md_desc"))%>';parent.window.close();"><%=rs("md_code")%></a></td>
          <td><%=rs("md_desc")%></td>
