...
<%
i = 1
sql = "SELECT md_id, md_code, md_desc, md_category, md_type, md_rcpprice FROM tblmodel where md_id is not null "
...
count = count + Showed
link = "&searchitem=" & searchitem & "&searchvalue=" & searchvalue & "&md_type=" & md_type 

%>
<table border="0" cellpadding="3" cellspacing="0" bordercolor="#CCCCCC">
  <tr> 
    <td class="style21"><font size="4"><strong>Item Code List</strong></font></td>
  </tr>
  <tr> 
    <td class="style21"><form name="form1" method="post" action="rm_cn_new_model.asp">
...
<select name="md_type" id="md_type">
  ...
  <tr> 
	...
	For j = 1 To pgCount
				...
				Else
					Response.Write " <a href='rm_cn_new_model.asp?num=" & (j-1) * row & link & "'>"& j &"</a>"
				End If
	...
	If Remain > row Then
	  Response.Write "<a href='rm_cn_new_model.asp?num=" & Showed+row & link & "'> Next >></a>"
	End If
  ...
  <tr> 
    <td valign="top"><table border="1" cellpadding="5" cellspacing="0" bordercolor="#E8E8E8">
        <tr valign="top" bgcolor="#88c0a7"> 
          ...
          <td width="14%" class='tktTotals'><strong>Unit Price</strong></td>
        </tr>
        <% 
...
%>
        <tr valign="top" bgcolor="<%=nbgcolor%>"> 
          <td nowrap><%=count%>.</td>
          <td><a href="javascript:parent.opener.document.forms['forminvoicedetail'].cnd_partcode.value='<%=rs("md_code")%>';parent.opener.document.forms['forminvoicedetail'].cnd_desc.value='<%=ChkString(rs("md_desc"))%>';parent.opener.document.forms['forminvoicedetail'].cnd_unitcost.value='<%=ChkString(rs("md_rcpprice"))%>';parent.opener.document.forms['forminvoicedetail'].cnd_subtotal.value='<%=ChkString(rs("md_rcpprice"))%>';parent.window.close();"><%=rs("md_code")%></a></td>
          <td><%=rs("md_desc")%></td>
          <td><%=rs("md_category")%></td>
          <td><%=rs("md_type")%></td>
          <td><%=rs("md_rcpprice")%></td>
        </tr>
        <%
count = count + 1 
i = i + 1
