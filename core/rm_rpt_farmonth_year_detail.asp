...
sql2 = "SELECT tbljob.job_id, tbljob.job_code, tbljob.job_count, tbljob.job_date, tbljob.job_cust_code, tbljob.job_cust_name, tbljob.job_cust_address, " & _
		...
		"and tbljob.job_tech_faulty_code='" & faulth_code & "' and tbljob.job_status='Posted' " 
...
link = "&job_tech_type=" & job_tech_type & "&faulth_code=" & faulth_code & "&jobmonth=" & jobmonth & "&jobyear=" & jobyear & "&job_tech_model=" & request("job_tech_model") & "&stype=" & stype & "&lastmonth=" & lastmonth 
%>
<table border="0" cellpadding="3" cellspacing="0" bordercolor="#CCCCCC">
  ...
  <tr> 
	...
	For j = 1 To pgCount
				...
				Else
					Response.Write " <a href='rm_rpt_farmonth_year_detail.asp?num=" & (j-1) * row & link & "'>"& j &"</a>"
				End If
	...
	If Remain > row Then
	  Response.Write "<a href='rm_rpt_farmonth_year_detail.asp?num=" & Showed+row & link & "'> Next >></a>"
	End If
