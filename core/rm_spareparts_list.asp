...
sql2 = "select * from (select tblmodel.md_code, tblmodel.md_desc, tblmodel.md_category, tblmodel.md_type, tblmodel.md_rcpprice,  " & _
		...
		"where (tblmodel.md_category = 'Parts' or tblmodel.md_category = 'Labour') " 
...
sql2 = sql2 & " and md_type='" & request("md_type") & "' "
end if

sql2 = sql2 & " group by tblmodel.md_code, tblmodel.md_desc, tblmodel.md_category, tblmodel.md_type, tblmodel.md_rcpprice)t "
...
<table border="0" cellpadding="3" cellspacing="0" bordercolor="#CCCCCC">
  ...
  <tr> 
    <td class="style21"><form name="form1" method="post" action="rm_spareparts_list.asp">
...
<select name="md_type" id="md_type">
  ...
  <tr> 
	...
	For j = 1 To pgCount
				...
				Else
					Response.Write " <a href='rm_spareparts_list.asp?num=" & (j-1) * row & link & "'>"& j &"</a>"
				End If
	...
	If Remain > row Then
	  Response.Write "<a href='rm_spareparts_list.asp?num=" & Showed+row & link & "'> Next >></a>"
	End If    %>
    </td>
  </tr>
  <tr> 
...
%>
        <tr valign="top" bgcolor="<%=nbgcolor%>"> 
          <td nowrap><%=count%>.</td>
          <td>
          <%if rs("md_code") <>"Labour" and rs("md_code")<> "Service" then %>
          <a href="javascript:parent.opener.document.forms['<%=formname%>'].<%=fieldname%>.value='<%=rs("md_code")%>';parent.opener.document.forms['<%=formname%>'].<%=fieldname1%>.value='<%=ChkString(rs("md_desc"))%>';parent.window.close();"><%=rs("md_code")%></a>
          <%else%>
          <%=rs("md_code")%>
          <%end if %></td>
          <td><%=rs("md_desc")%></td>
