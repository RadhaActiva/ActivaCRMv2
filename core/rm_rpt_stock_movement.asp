...
if request("job_date_to") <> "" then
   ...
   job_date_to = chkdate(date())
    ...
    sql12= "Select md_code from tblmodel where tblmodel.md_code='" & searchvalue & "'"
    set rs12 = server.CreateObject("adodb.recordset")
    rs12.ActiveConnection = strconnect
    rs12.Source = sql12
    rs12.Open    
    if rs12.eof then
     stock_item_exist=false 'used to check for invalid stock code
    end if
    ...
    "stk_total_cost from tblstock_movement a where stk_code_id = '" & searchvalue & "' and stk_logby = '" &  Request.Cookies("GAPS")("sloginid") & "' order by stk_date"
...
if searchvalue = "" then
    sql6= "delete FROM tblstock_movement where stk_logby = '" &  Request.Cookies("GAPS")("sloginid") & "'"
...
if searchvalue <> "" and Request.QueryString("post") = "yes" and stock_item_exist= true then ''this code should be executed on the 1st time loading data only

    'Delete all data
    sql8 = "delete from tblstock_movement where stk_logby = '" &  Request.Cookies("GAPS")("sloginid") & "'"
    ...
    if whchk = "No" then 
        sql1="insert into tblstock_movement (stk_code_id, stk_date, stk_doc_type, stk_doc_no, stk_qty, stk_bf_qty, stk_avg_cost, stk_purchase_cost, stk_logby) " & _
            "select top 1 a.stk_itm_code,'2023-01-01','Balance','Balance C/F','0.00',(select sum (a.stk_qty) from tblstocktran a " & _
            "where a.stk_itm_code ='" & searchvalue & "' and cast(a.stk_date as date) <='01/01/2023' and a.stk_reference='" & wh_code & "') as 'bal_cf', (select TOP 1 md_averagecost from tblmodel_avgcost where md_code = '" & searchvalue & "' and CAST(tblmodel_avgcost.md_date as date) <= '20230101' " & _ 
            "order by md_date desc), " & _ 
            "(select '0.00' from tblmodel where tblmodel.md_code='" & searchvalue & "') as purchase_price, '" &  Request.Cookies("GAPS")("sloginid") & "' from tblstocktran a where a.stk_itm_code ='" & searchvalue & "' " & _
            " and CAST(a.stk_date as date) < '" & job_date_to & "' and a.stk_reference='" & wh_code & "'"
    else
        sql1="insert into tblstock_movement (stk_code_id, stk_date, stk_doc_type, stk_doc_no, stk_qty, stk_bf_qty, stk_avg_cost, stk_purchase_cost, stk_logby) " & _
            "select top 1 a.stk_itm_code,'2023-01-01','Balance','Balance C/F','0.00',(select sum (a.stk_qty) from tblstocktran a " & _
            "where a.stk_itm_code ='" & searchvalue & "' and cast(a.stk_date as date) <='01/01/2023') as 'bal_cf', (select TOP 1 md_averagecost from tblmodel_avgcost where md_code = '" & searchvalue & "' and CAST(tblmodel_avgcost.md_date as date) <= '20230101' " & _ 
            "order by md_date desc), " & _ 
            "(select '0.00' from tblmodel where tblmodel.md_code='" & searchvalue & "') as purchase_price, '" &  Request.Cookies("GAPS")("sloginid") & "' from tblstocktran a where a.stk_itm_code ='" & searchvalue & "' " & _
            "and CAST(a.stk_date as date) < '" & job_date_to & "'"
    end if 
    ...
    'if not rs1.eof then 'to ensure the stk_itm_code does exist otherwise skip below
        if whchk = "No" then 
                ...
                "where  CAST(stk_date as date) > '01/01/2023' and stk_itm_code = '" & searchvalue & "' and a.stk_reference='" & wh_code & "' order by stk_date "
            else
                sql3= "insert into tblstock_movement (stk_code_id, stk_date, stk_doc_type, stk_doc_no, stk_qty,stk_bf_qty,stk_avg_cost, stk_purchase_cost, stk_logby)" & _
                "select a.stk_itm_code, a.stk_date,a.stk_type, stk_voucherno,stk_qty,'0.00','0.00',stk_cost_price, '" &  Request.Cookies("GAPS")("sloginid") & "' from tblstocktran a " & _
                "where  CAST(stk_date as date) > '01/01/2023' and stk_itm_code = '" & searchvalue & "' order by stk_date "
            end if 
    'end if 
    ...
    if rs4("stk_doc_no") = "Balance C/F" then
        ...
        if rs4("stk_doc_type") = "Stock-In" then
           ...
           ' if totalpurchase_cost = 0 then 'in case no totalpurchasecost or purchase cost use the last avg cost
        ...
        total_cost =  round(totalpurchase_cost,2) + round(prev_total_cost,2)
    ...
    'carry-fwd prev weighted avg cost except for Stock-in transactions.
   ' if totalpurchase_cost = 0 and  rs4("stk_doc_type") = "Stock-In" then 'logic only for WWH07-001
...
{
	document.form1.action = "rm_rpt_stock_movement.asp?post=yes";
	document.form1.submit();
        ...
        <tr>
          <td><table width="97%" border="0" align="center" cellpadding="0" cellspacing="0">
                ...
                <tr>
                  ...
                  <td colspan="2" valign="top" bgcolor="#FFFFFF"><table width="100%" border="0" cellpadding="0" cellspacing="0">
                    <tr>
                      ...
                      <td width="20%" align="center" class="titlegrey1"><a href="rm_rpt_stock_movement_excel.asp?searchvalue=<%=searchvalue%>&wh_code=<%=wh_code%>&whchk=<%=whchk%>&job_date_to=<%=job_date_to%>" target="_blank"><img src="images/excel.jpg" width="57" height="21" border="0"/></a></td>
                    </tr>
                  </table></td>
                </tr>
                <form id="form1" name="form1" method="post" action="rm_rpt_stock_movement.asp?post=yes">
                     <tr>
                  <td height="30" align="left" bgcolor="#FFFFFF" width="45%"><strong><font color="#000000"><strong>Up to
             ...
             calendar = window.open('date.asp',null,'WIDTH=185,HEIGHT=203,status=no,toolbar=no,menubar=no,location=no,scrollbars=no,resizable=1,top=0,right=0')"> <img src="images/calender.gif" width="20" height="19" border="0" align="absmiddle" /></a></strong></font></strong> Date must be (dd-MMM-yyyy) eg: 21-May-2022 </td>
                ...
                <tr><td height="30" bgcolor="#FFFFFF"> <Strong>Stock Code  </Strong>
                   ...
                   <td width="80%" height="30" align="left" bgcolor="#FFFFFF"><input type="button" name="button2" id="button" value="Display Report" onclick="javascript:DisplayReport();" /></td></tr>
                 </form>
...
%>                   
                     <tr bgcolor="<%=nbgcolor%>">
                      <td align="left" nowrap="nowrap" bgcolor="#F3F3F3"><strong> <%=chkdate(rs4("stk_date"))%></strong></td>
                      ...
                      <td align="center" nowrap="nowrap" bgcolor="#F3F3F3"><%=chknumber2(rs4("stk_purchase_cost"))%></td>
                      <td align="center" nowrap="nowrap" bgcolor="#F3F3F3"><%=chknumber2(rs4("stk_purchase_totalcost"))%></td>
                      <td align="right" nowrap="nowrap" bgcolor="#F3F3F3"><%=chknumber2(rs4("stk_avg_cost"))%></td>
                      <td align="right" nowrap="nowrap" bgcolor="#F3F3F3"><%=chknumber2(rs4("stk_total_cost"))%></td>
                    </tr>
<%
