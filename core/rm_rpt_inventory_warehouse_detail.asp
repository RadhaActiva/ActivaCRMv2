...
sql = "SELECT stk_id, stk_voucherno, stk_reference, stk_date, stk_type, stk_itm_code, stk_fromwarehouse, stk_towarehouse, stk_desc, stk_qty, " & _
	  ...
	  "and stk_date >= '" & ChkDateYYYYMMDD(job_date_from) & "' and stk_date <= '" & ChkDateYYYYMMDD(job_date_to) & "' "
...
count = count + Showed
link = "&stk_itm_code=" & stk_itm_code & "&stk_reference=" & stk_reference & "&job_date_from=" & job_date_from & "&job_date_to=" & job_date_to & "&stype=" & stype 
%>
<table border="0" cellpadding="3" cellspacing="0" bordercolor="#CCCCCC">
  ...
  <tr> 
    <td class="style21"><form name="form1" method="post" action="rm_rpt_inventory_warehouse_detail.asp">
<strong>Store</strong>
...
<input type="submit" name="Submit" value="Submit">
  ...
  <tr> 
	...
	For j = 1 To pgCount
				...
				Else
					Response.Write " <a href='rm_rpt_inventory_warehouse_detail.asp?num=" & (j-1) * row & link & "'>"& j &"</a>"
				End If
	...
	If Remain > row Then
	  Response.Write "<a href='rm_rpt_inventory_warehouse_detail.asp?num=" & Showed+row & link & "'> Next >></a>"
	End If
