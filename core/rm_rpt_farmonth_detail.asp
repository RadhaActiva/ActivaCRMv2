...
sql2 = "SELECT tbljob.job_id, tbljob.job_code, tbljob.job_count, tbljob.job_date, tbljob.job_cust_code, tbljob.job_cust_name, tbljob.job_cust_address, " & _
		...
		"tbljob.job_totalAmt, tbljob.job_repair_date, tbljob.job_return_tech_date, tbljob.job_office_issueRemark, tbljob.job_office_supervisor, tbljob.job_posteddate, " & _
...
link = "&job_tech_type=" & job_tech_type & "&faulth_code=" & faulth_code & "&jobmonth=" & jobmonth & "&jobyear=" & jobyear & "&job_tech_model=" & request("job_tech_model") & "&stype=" & stype & "&lastmonth=" & request("lastmonth")
%>
<table border="0" cellpadding="3" cellspacing="0" bordercolor="#CCCCCC">
  <tr> 
    <td width="593" class="style21"><font size="4"><strong>Job Listing</strong></font></td>
    <td width="82" align="center" class="style21"><a href="rm_rpt_farmonth_detail_excel.asp?jobmonth=<%=jobmonth%>&jobyear=<%=jobyear%>&faulth_code=<%=faulth_code%>&job_tech_type=<%=job_tech_type%>&job_tech_model=<%=job_tech_model%>&stype=<%=stype%>&lastmonth=<%=lastmonth%>" target="_blank"><img src="images/excel.jpg" width="57" height="21" border="0"></a></td>
  </tr>
  <tr> 
	...
	For j = 1 To pgCount
				...
				Else
					Response.Write " <a href='rm_rpt_farmonth_detail.asp?num=" & (j-1) * row & link & "'>"& j &"</a>"
				End If
	...
	If Remain > row Then
	  Response.Write "<a href='rm_rpt_farmonth_detail.asp?num=" & Showed+row & link & "'> Next >></a>"
	End If
