...
<table border="0" cellpadding="3" cellspacing="0" bordercolor="#CCCCCC">
  ...
  <tr> 
    <td class="style21"><form name="form1" method="post" action="rm_do_new_customer.asp">
  ...
  <tr> 
	...
	For j = 1 To pgCount
				...
				Else
					Response.Write " <a href='rm_do_new_customer.asp?num=" & (j-1) * row & link & "'>"& j &"</a>"
				End If
	...
	If Remain > row Then
	  Response.Write "<a href='rm_do_new_customer.asp?num=" & Showed+row & link & "'> Next >></a>"
	End If
...
%>
        <tr valign="top" bgcolor="<%=nbgcolor%>"> 
          <td><%=count%>.</td>
          <td>
          <a href="javascript:parent.opener.document.forms['formorder'].do_cust_code.value='<%=rs("cust_code")%>';parent.opener.document.forms['formorder'].do_cust_name.value='<%=rs("cust_name")%>';parent.opener.document.forms['formorder'].do_cust_address.value='<%=rs("cust_address")%>';parent.opener.document.forms['formorder'].do_cust_postcode.value='<%=rs("cust_postcode")%>';parent.opener.document.forms['formorder'].do_cust_state.value='<%=rs("cust_state_id")%>';parent.opener.document.forms['formorder'].do_cust_city.value='<%=rs("cust_city_id")%>';parent.opener.document.forms['formorder'].do_cust_email.value='<%=rs("cust_email")%>';parent.opener.document.forms['formorder'].do_cust_tel1.value='<%=rs("cust_tel1")%>';parent.opener.document.forms['formorder'].do_cust_tel2.value='<%=rs("cust_tel2")%>';parent.opener.document.forms['formorder'].do_remark.value='<%=rs("cust_attention")%> <%=rs("cust_pic")%>';parent.opener.document.forms['formorder'].do_onlineWrtyNo.value='<%=rs("warrantyno")%>';parent.opener.document.forms['formorder'].do_SN_no.value='<%=rs("serialno")%>';parent.opener.document.forms['formorder'].do_purchase_date.value='<%=chkdate(rs("purchase_date"))%>';parent.window.close();"><%=rs("cust_code")%></a><br>
          <%=rs("cust_status")%>
          </td>
