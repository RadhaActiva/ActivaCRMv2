...
sf_fromwarehouse=request("sf_fromwarehouse")
%>
<html>
<head>
...
sql ="select tblwarehouse_stock.wst_itm_code, tblmodel.md_code,tblmodel.md_desc, tblmodel.md_category, tblmodel.md_type,tblwarehouse_stock.wst_itm_current_qty, md_averageecost " & _
	...
	"tblwarehouse_stock.wst_itm_code=tblmodel.md_code where tblmodel.md_category = 'Parts' and " & searchitem & " like '%" & searchvalue& "%' and tblwarehouse_stock.wst_wh_code = '" & sf_fromwarehouse & "' " & _
...
<table border="0" cellpadding="3" cellspacing="0" bordercolor="#CCCCCC">
  <tr> 
    <td class="style21"><font size="4"><strong>Stock List for Store: <%=sf_fromwarehouse%></strong></font></td>
  </tr>
  <tr> 
    <td class="style21"><form name="form1" method="post" action="rm_stocktfr_new_item.asp">
<strong>Model Type</strong>
<select name="md_type" id="md_type">
          <option value="" <%if md_type = "" then response.write " selected"%>>All</option>
        ...
        <input type="hidden" name="sf_fromwarehouse" value="<%=sf_fromwarehouse%>">
  ...
  <tr> 
	...
	For j = 1 To pgCount
				...
				Else
					Response.Write " <a href='rm_stocktfr_new_item.asp?num=" & (j-1) * row & link & "'>"& j &"</a>"
				End If
	...
	If Remain > row Then
	  Response.Write "<a href='rm_stocktfr_new_item.asp?num=" & Showed+row & link & "'> Next >></a>"
	End If
...
%>
        <tr valign="top" bgcolor="<%=nbgcolor%>"> 
          <td nowrap><%=count%>.</td>
          <td nowrap><a href="javascript:parent.opener.document.forms['formdodetail'].sfd_itm_code.value='<%=rs("md_code")%>';parent.opener.document.forms['formdodetail'].sfd_itm_desc.value='<%=ChkString(rs("md_desc"))%>';parent.window.close();"><%=rs("md_code")%></a></td>
          <td><%=rs("md_desc")%></td>
