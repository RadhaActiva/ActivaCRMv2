...
sql2 = "SELECT tbljob.job_id, tbljob.job_code, tbljob.job_count, tbljob.job_date, tbljob.job_cust_code, tbljob.job_cust_name, tbljob.job_cust_address, " & _
		...
		"and tbljob.job_tech_code='" & tech_code & "' " & _
	...
	if stype = "Accepted1-3" then 
	   sql2 = sql2 & " and DATEDIFF(tbljob.job_JS_receiveddate, tbljob.job_submitteddate) > -1 and DATEDIFF(tbljob.job_JS_receiveddate, tbljob.job_submitteddate) < 4 "
	elseif stype = "Accepted4-6" then    
	   sql2 = sql2 & " and DATEDIFF(tbljob.job_JS_receiveddate, tbljob.job_submitteddate) > 3 and DATEDIFF(tbljob.job_JS_receiveddate, tbljob.job_submitteddate) < 7 "
	elseif stype = "Accepted7above" then  
	   sql2 = sql2 & " and DATEDIFF(tbljob.job_JS_receiveddate, tbljob.job_submitteddate) > 6 "
	elseif stype = "Done1-3" then  
	   sql2 = sql2 & " and DATEDIFF(tbljob.job_donedate, tbljob.job_JS_receiveddate) > -1 and DATEDIFF(tbljob.job_donedate, tbljob.job_JS_receiveddate) < 4 "
	elseif stype = "Done4-6" then  
	   sql2 = sql2 & " and DATEDIFF(tbljob.job_donedate, tbljob.job_JS_receiveddate) > 3 and DATEDIFF(tbljob.job_donedate, tbljob.job_JS_receiveddate) < 7 "
	elseif stype = "Done7above" then  
	   sql2 = sql2 & " and DATEDIFF(tbljob.job_donedate, tbljob.job_JS_receiveddate) > 6 "
	elseif stype = "Posted" then  
...
link = "&job_date_from=" & job_date_from & "&job_date_to=" & job_date_to & "&tech_code=" & tech_code & "&stype=" & stype 
%>
<table border="0" cellpadding="3" cellspacing="0" bordercolor="#CCCCCC">
  ...
  <tr> 
	...
	For j = 1 To pgCount
				...
				Else
					Response.Write " <a href='rm_rpt_monthtech_detail.asp?num=" & (j-1) * row & link & "'>"& j &"</a>"
				End If
	...
	If Remain > row Then
	  Response.Write "<a href='rm_rpt_monthtech_detail.asp?num=" & Showed+row & link & "'> Next >></a>"
	End If
